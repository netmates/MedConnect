...
{
    private readonly UpdateSpecializationValidator _validator = new();

    private static UpdateSpecializationDto ValidDto(string? name = null)
        => new()
        {
            Name = name ?? "Терапия"
        };
    ...
    public void Validate_WithEmptyOrWhitespaceName_Fails(string name)
    {
        ...
        Assert.Contains(
            result.Errors,
            e => e.PropertyName == nameof(UpdateSpecializationDto.Name)
